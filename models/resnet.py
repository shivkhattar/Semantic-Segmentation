...
class ResNetCls(nn.Module):
    def __init__(self, block, layers, num_classes=1000,
        ...
        self._norm_layer = partial(build_normalization_layer, norm_cfg, layer_only=True)

        if act_cfg is None:
            act_cfg = dict(type='Relu', inplace=True)
        self._act_layer = partial(build_activation_layer, act_cfg, layer_only=True)
...
class ResNet(ResNetCls):
    def __init__(self, arch, replace_stride_with_dilation=None, multi_grid=None,
                 pretrain=True, norm_cfg=None, act_cfg=None):
        cfg = MODEL_CFGS[arch]
        super().__init__(
            cfg['block'],
            cfg['layer'],
